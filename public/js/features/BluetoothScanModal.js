...
class BluetoothScanModal {
  ...
  handleScanComplete(data) {
    ...
    this.availableDevices = allDevices.filter((device) => {
      ...
      const isAlreadyPaired = this.pairedDevices.some((paired) => paired.address === deviceId);
      return !isAlreadyPaired;
    });

    this.logger.info(
      'BluetoothScanModal',
      `Scan complete: ${allDevices.length} devices found, ${this.availableDevices.length} available (${allDevices.length - this.availableDevices.length} already paired)`
    );
  ...
  handleDeviceUnpaired(data) {
    ...
    this.pairedDevices = this.pairedDevices.filter((d) => d.address !== deviceId);
  ...
  renderBluetoothDisabled() {
    ...
    const warnBg = isDark
      ? 'linear-gradient(135deg, #fff3cd 0%, #ffe5b4 100%)'
      : 'linear-gradient(135deg, rgba(240, 180, 41, 0.12) 0%, rgba(240, 180, 41, 0.08) 100%)';
