// ============================================================================
// File: public/js/features/midi-editor/MidiEditorChannelPanel.js
...
class MidiEditorChannelPanel {
  ...
  toggleChannel(channel) {
    ...
    m.log(
      'info',
      `Toggled channel ${channel}. Active channels: [${Array.from(m.activeChannels).join(', ')}]`
    );
  ...
  async updateTablatureButton() {
    ...
    if (m.activeChannels.size === 1) {
      ...
      const gmMatch = channelInfo
        ? MidiEditorChannelPanel.getStringInstrumentCategory(channelInfo.program)
        : null;
  ...
  _suggestStringInstrumentConfig(channel, gmMatch, channelInfo) {
    ...
    banner.innerHTML = `
            <span>${m.tHtml('tablature.gmDetected', { instrument: channelInfo.instrument })}</span>
    ...
    const toolbar =
      m.container?.querySelector('.tablature-toolbar') ||
  ...
  updateInstrumentSelector() {
    ...
    if (m.activeChannels.size === 0) {
      ...
      if (instrumentLabel) {
        instrumentLabel.textContent = m.t('midiEditor.multipleChannels', {
          count: m.activeChannels.size
        });
