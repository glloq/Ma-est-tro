(function() {
    ...
    ISMNavigation._addTab = async function() {
        ...
        for (let ch = 0; ch < 16; ch++) {
            ...
            const tooltip = isUsed ? `${instName} — ${this.t('instrumentManagement.channelUsed') || 'occupé'}` : (isDrum ? 'Canal percussion' : `Canal ${ch + 1} — libre`);
            gridHtml += `<button type="button" class="add-inst-channel-btn ${isUsed ? 'used' : ''} ${isDrum ? 'drum' : ''}" data-channel="${ch}" ${isUsed ? 'disabled' : ''} style="border-color: ${colors[ch]};" title="${tooltip}">
