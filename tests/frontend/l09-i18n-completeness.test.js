...
const TECH = new Set([
  'midi','usb','ble','bluetooth','wifi','wi-fi','rtp','applemidi','osc','mqtt','artnet','art-net',
  'sacn','e1.31','dmx','gpio','uart','spi','i2c','led','leds','ws2812','neopixel','wled','tasmota',
  'esphome','hue','philips','qlc+','qlc','touchdesigner','raspberry','pi','rgb','rgbw','http','https',
  'rest','tcp','udp','ip','ipv4','mac','url','id','uuid','api','json','xml','csv','sf2','sfz','soundfont',
  'gm','gm2','gs','xg','sysex','nrpn','rpn','pb','cc','bpm','ms','hz','khz','db','rssi','azerty','qwerty',
  'ok','on','off','tab','edit','wind','drum','tap','pwm','ssid','lan','wlan','dhcp','ssh','cpu','ram',
  'sd','os','ui','pdf','png','svg','ntp','mdns','dns','vpn','ac','dc','v','ma','a','w','fps','px','hh',
  'ch','vel','vol','min','max','num','no.','msgs','sec','s','m','h','ko','mo','go','kb','mb','gb','ms.',
  'set','din','trs','jack','xlr','ttl','rx','tx','sn','fw','hw','sw','bt',
  'do','re','ré','mi','fa','sol','la','si','ti','c','d','e','f','g','b','x','y','z','n','t',
  'staccato','legato','glissando','tremolo','vibrato','portamento','crescendo','arpeggio','ostinato',
  'ackick','kick','stick','snare','clap','esnare','lflrtom','clhh','hflrtom','pdlhh','lowtom','ophh',
  'lmtom','hmtom','crash','crash1','hitom','ride','ride1','china','rdbell','tamb','splash','cowbell',
  'crash2','vibra','ride2','hibong','lobong','mtcnga','opcnga','locnga','hitimb','lotimb','hiago',
  'loago','cabasa','maraca','swhstl','lwhstl','sguiro','lguiro','claves','hiwdbk','lowdbk','mtcuic',
  'opcuic','mttri','optri','ctrl','multi','universe','subnet','port','segment'
]);

const LOAN = new Set([
  'piano','tempo','solo','pan','modulation','expression','test','instrument','instruments',
  'transposition','version','format','formats','import','export','note','notes','volume','interface',
  'mode','position','configuration','total','application','options','option','filter','filtre',
  'standard','normal','type','global','local','audio','video','stereo','mono','master','pulse','flash',
  'effect','effets','reset','stop','pause','play','start','signal','scanner','firmware','action',
  'accent','chorus','reverb','delay','sustain','attack','release','decay','tension','contact','base',
  'sequence','panel','console','micro','metronome','gamme','octave','tonic','accord','texture',
  'harmonica','banjo','guitare','guitar','violin','viola','cello','trombone','trumpet','clarinet',
  'saxophone','flute','tuba','timbre','tone','tonal','tutorial','info','description','notation',
  'instant','table','menu','canal','general','service','status','sensor','moderato','allegro',
  'adagio','presto','de'
]);
...
function flatten(obj, prefix = '') {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (v !== null && typeof v === 'object' && !Array.isArray(v)) Object.assign(out, flatten(v, full));
...
const data = Object.fromEntries(
  locales.map((l) => [l, flatten(JSON.parse(fs.readFileSync(path.join(LOCALES_DIR, `${l}.json`), 'utf-8')))])
);
const en = data.en;
...
function measure(locale, keySet = keys) {
  ...
  return { identical, legit, floor, ceiling, mid: (floor + ceiling) / 2, margin: (ceiling - floor) / 2 };
}
...
const RATCHET = {
  fr: 91, eo: 88, bn: 87, th: 87, es: 85, de: 84, it: 82, vi: 80, id: 79, pl: 78,
  cs: 78, sv: 77, pt: 77, fi: 77, no: 77, hu: 77, nl: 77, tr: 77, da: 77, uk: 76,
  ja: 76, ko: 76, 'zh-CN': 76, ru: 76, el: 75, tl: 72, hi: 71
};
